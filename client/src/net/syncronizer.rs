...
use crate::net::{self, remote_player::RemotePlayer};
...
impl scene::Node for NetSyncronizer {
    fn ready(_: RefMut<Self>) {
        let idle = async move {
            loop {
                net::send_bin(message::Idle::OPCODE, &message::Idle);
    ...
    fn update(mut node: RefMut<Self>) {
        {
            ...
            let network_frame =
                get_time() - node.network_cache.last_send_time > (1. / 15.0) as f64;
            ...
            if shooting || network_frame {
                ...
                if node.network_cache.sent_position != state.0 {
                    ...
                    net::send_bin(message::Move::OPCODE, &message::Move(state.0));
                }

                // if node.network_cache.sent_death_state != player.is_dead() {
                //     if player.is_dead() {
                //         net::send_bin(message::Died::OPCODE, &message::Died);
                //     }
        ...
        while let Some(event) = net::events() {
            match event {
                net::Event::Leave(leaver) => {
        ...
        while let Some(msg) = net::try_recv() {
            if let Some(other) = node.others.get(&msg.user_id) {
                ...
                match msg.opcode as i32 {
                    message::Move::OPCODE => {
                        let message::Move(data) = bincode::deserialize(&msg.data).unwrap();
                    ...
                    //     let message::SelfDamage(_health) = bincode::deserialize(&msg.data).unwrap();
                    // }
                    // message::Died::OPCODE => {
                    //     let mut resources = storage::get_mut::<Resources>().unwrap();
                    ...
                    // }
                    // message::SpawnItem::OPCODE => {
                    //     let message::SpawnItem { id, x, y } = bincode::deserialize(&msg.data).unwrap();
                    ...
                    // }
                    // message::DeleteItem::OPCODE => {
                    //     let message::DeleteItem { id } = bincode::deserialize(&msg.data).unwrap();
